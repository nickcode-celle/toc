...
function finishXY(owner:PlayerId,pos:number){
  const angle=(BASE_POSITION[owner]/TRACK_SIZE)*Math.PI*2-Math.PI/2;
  const r=31-pos*6.3; return {x:50+r*Math.cos(angle),y:50+r*Math.sin(angle)};
}
...
function Board({game,selectedCardId,selectedMarbleId,onMarble,onDestination,mode}:{game:MachineGame;selectedCardId:string|null;selectedMarbleId:string|null;onMarble:(id:string)=>void;onDestination:(m:LegalMove)=>void;mode:string}){
  ...
  const controlled=new Set(legal.flatMap(m=>m.type==="SEVEN"?m.parts.map(p=>p.marbleId):m.type==="JACK"?[m.marbleId,m.targetMarbleId]:[m.marbleId]));
  const destinations=selectedMarbleId?legal.filter(m=>m.type!=="JACK"&&m.type!=="SEVEN"&&"marbleId" in m&&m.marbleId===selectedMarbleId).map(m=>({move:m,xy:destinationForMove(game.state,m)})).filter(x=>x.xy):[];
  return <div className="board"><div className="logo">TOC<span>♠</span></div>{holes.map(i=>{const p=trackXY(i);return <i key={i} className="hole" style={{left:`${p.x}%`,top:`${p.y}%`}}/>})}
    ...
    {game.state.marbles.map((m,i)=>{const p=marbleXY(m,i);const can=game.phase==="HUMAN_TURN"&&controlled.has(m.id);return <button key={m.id} aria-label={m.id} className={`marble ${playerClass[m.owner]} ${selectedMarbleId===m.id?"selected-marble":""} ${mode==="EASY"&&can?"can-play":""}`} style={{left:`${p.x}%`,top:`${p.y}%`}} onClick={()=>can&&onMarble(m.id)}/>})}
    {destinations.map(({move,xy},i)=><button key={i} className={`destination ${mode==="EXPERT"?"invisible-destination":""}`} style={{left:`${xy!.x}%`,top:`${xy!.y}%`}} onClick={()=>onDestination(move)} />)}
  </div>;
}
function App(){
  const [game,setGame]=useState<MachineGame>(()=>createMachineGame(createInitialGameState(),0));
  const [mode,setMode]=useState("NORMAL");
  const [selectedCardId,setSelectedCardId]=useState<string|null>(null); const [selectedMarbleId,setSelectedMarbleId]=useState<string|null>(null); const [jackSource,setJackSource]=useState<string|null>(null);
  ...
  function settle(g:MachineGame){ g=advanceMachines(g); setGame(g); setSelectedCardId(null);setSelectedMarbleId(null);setJackSource(null); }
  function exchange(id:string){ settle(submitHumanExchange(game,id)); }
  function chooseCard(id:string){ if(game.phase!=="HUMAN_TURN")return; setSelectedCardId(id);setSelectedMarbleId(null);setJackSource(null); }
  function chooseMarble(id:string){
    ...
    if(card?.rank==="7"){
      const plans=moves.filter((m):m is Extract<LegalMove,{type:"SEVEN"}>=>m.type==="SEVEN"&&m.parts.length===1&&m.parts[0].marbleId===id); if(plans.length===1)settle(submitHumanMove(game,plans[0])); else setSelectedMarbleId(id); return;
    }
