...
export function chooseBaselineExchange(state:GameState,player:PlayerId):string{
 ...
 const enemies:PlayerId[]=state.players.filter(p=>p.team===enemyTeam).map(p=>p.id); const enemyFinished=enemies.reduce<number>((s,o)=>s+finished(state,o),0);
 const enemyNear=enemies.some(o=>rel(state,o).some(x=>x>=58&&x<64));
 const order=nextPlayers(player); const firstEnemy=order.findIndex(p=>state.players.find(x=>x.id===p)!.team===enemyTeam); const partnerActsBeforeThreat=order.indexOf(partner)>=0&&(firstEnemy<0||order.indexOf(partner)<firstEnemy);
 function gift(c:Card){
   let v=rankValue(c);
   if(ph>0&&(c.rank==="A"||c.rank==="K"))v+=130;
   if(pf>=2&&c.rank==="7")v+=70;
   if(partnerTrack.some(x=>x===0)&&c.rank==="4")v+=55;
