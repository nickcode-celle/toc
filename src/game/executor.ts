...
function detectWinner(state: GameState): TeamId | null { for (const team of [0, 1] as TeamId[]) { const owners = state.players.filter((p) => p.team === team).map((p) => p.id); if (state.marbles.filter((m) => owners.includes(m.owner)).every((m) => m.zone === "FINISH")) return team; } return null; }
...
function executeSevenPart(state: GameState, marble: Marble, steps: number): void {
  ...
  if (marble.zone === "TRACK" && marble.trackPosition !== null) {
    ...
    for (let i = 1; i <= steps; i += 1) {
      ...
      const target = marbleAtTrackPosition(state, pos, marble.id); if (target) sendHome(target);
  ...
  } else { marble.zone = "TRACK"; marble.trackPosition = destination.trackPosition; marble.finishPosition = null; marble.qualifiedForFinish = qualifies; }
}
...
export function executeMove(state: GameState, move: LegalMove): GameState {
  ...
  if (move.type === "SEVEN") { for (const part of move.parts) { const m = next.marbles.find((x) => x.id === part.marbleId); if (!m) throw new Error("Unknown seven marble"); executeSevenPart(next, m, part.steps); } }
